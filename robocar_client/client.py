...
def main() -> None:
    ...
    if args.behavior_name:
        ...
        behavior_name = available[0]
    spec = env.behavior_specs[behavior_name]
    print(
        f"Connecté à {behavior_name} | obs: {len(spec.observation_specs)} | actions cont.: {spec.action_spec.continuous_size} | actions disc.: {spec.action_spec.discrete_size}"
    )
    if spec.action_spec.discrete_size:
        print(f"Actions discrètes: branches={spec.action_spec.discrete_branches}")

    # Identifie l'agent principal pour nommer le fichier de log
    first_decision, first_terminal = env.get_steps(behavior_name)
    if len(first_decision):
        main_agent_id = int(first_decision.agent_id[0])
    elif len(first_terminal):
        main_agent_id = int(first_terminal.agent_id[0])
    else:
        main_agent_id = 0
    ...
    with DataLogger(args.output_dir, agent_id=main_agent_id) as logger:
        try:
            while True:
                # Récupère états courants
                decision_steps, terminal_steps = env.get_steps(behavior_name)
                if step_count % 50 == 0:
                    print(f"step {step_count}: agents actifs={len(decision_steps)} terminés={len(terminal_steps)}")
                ...
                if args.max_steps and step_count >= args.max_steps:
                    ...
                    break
                print(f"Episode {episode} | Step {step_count} | Agents actifs: {len(decision_steps)}", end="\r")
